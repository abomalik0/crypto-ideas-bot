...
# =====================================================
#   Advanced Schools Engine (V2) — Detailed & Separated
#   NOTE:
#     - ICT مدرسة مستقلة عن SMC (مش نفس المدرسة)
#     - هذا القسم لا يحذف أي كود قديم؛ مجرد إضافة + تفعيل اختيارى
# =====================================================
...
def _school_snapshot_core(symbol: str = "BTCUSDT") -> dict:
    """
    لقطة خفيفة موحّدة لإظهار أرقام بسيطة داخل تقارير المدارس.
    نحاول قدر الإمكان استخدام compute_smart_market_snapshot(symbol) إن كان مدعوم.
    """
    ...
    alert_level = (snap or {}).get("alert_level") or {}
    ...
    out = {
        ...
        "strength": metrics.get("strength_label") or metrics.get("trend_label") or "-",
        "liquidity_pulse": metrics.get("liquidity_pulse") or "-",
        "risk_level": risk.get("level") or "unknown",
        "risk_score": risk.get("score") if isinstance(risk.get("score"), (int, float)) else None,
        "risk_emoji": risk.get("emoji") or "",
        ...
        "zones": zones,
        "alert_level": alert_level,
        "fmt": {"num": _f_num, "price": _f_price},
        "raw_snapshot": snap,
    }
    return out


def _school_risk_line(s: dict) -> str:
    """
    سطر مخاطر موحّد للمدارس (تعليمى).
    """
    try:
        level = s.get("risk_level")
        emoji = s.get("risk_emoji") or ""
        score = s.get("risk_score")
        if score is None:
            return f"⚠️ <b>المخاطرة الآن:</b> {emoji} <b>{level}</b>"
        # score فى نظامك أحيانًا 0..100؛ وأحيانًا 0..10 — نعرضه كما هو
        return f"⚠️ <b>المخاطرة الآن:</b> {emoji} <b>{level}</b> (score≈{score})"
    except Exception:
        return "⚠️ <b>المخاطرة الآن:</b> غير متاحة"


def _zones_block(s: dict) -> str:
    z = s.get("zones") or {}
    items = [
        ("Downside 1", z.get("downside_zone_1")),
        ("Downside 2", z.get("downside_zone_2")),
        ("Upside 1", z.get("upside_zone_1")),
        ("Upside 2", z.get("upside_zone_2")),
    ]
    lines = []
    for label, zone in items:
        ...
        try:
            ...
            lines.append(f"• {label}: بين <b>{lo:,.0f}$</b> و <b>{hi:,.0f}$</b>")
        except Exception:
            continue
    if not lines:
        return ""
    return "🗺 <b>مناطق حركة تقديرية (تعليمية):</b>\n" + "\n".join(lines)


def _school_market_header(s: dict) -> str:
    fnum = s["fmt"]["num"]
    fpr = s["fmt"]["price"]
    sym = s["symbol"]
    price = s.get("price")
    ...
    liq = s.get("liquidity_pulse")
    speed = s.get("speed")
    accel = s.get("accel")
    conf = s.get("direction_conf")
    return (
        f"💠 <b>Market Snapshot:</b> <code>{sym}</code>\n"
        f"• السعر: <b>${fpr(price)}</b> | 24h: <b>{fnum(change)}%</b> | مدى اليوم: <b>{fnum(rng)}%</b>\n"
        f"• التقلب: <b>{fnum(vol,1)}</b>/100 | قوة الحركة: <b>{strength}</b> | نبض السيولة: <b>{liq}</b>\n"
        f"• Pulse: speed={fnum(speed,1)}/100 | accel={fnum(accel,2)} | conf={fnum(conf,1)}%"
    )


def _school_checklist(title: str, points: list[str]) -> str:
    pts = "\n".join([f"✅ {p}" for p in points if p])
    return f"🧾 <b>{title}</b>\n{pts}" if pts else ""


def _build_school_ict(s: dict) -> str:
    """
    ICT (Inner Circle Trader) مدرسة مستقلة: Killzones + Liquidity + Order Blocks + FVG + Displacement + OTE.
    """
    sym = s["symbol"]
    header = (
        "🏴‍☠️ <b>مدرسة ICT — Inner Circle Trader</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية شراء/بيع.</i>\n"
        "🧠 ICT تركّز على: <b>السيولة</b> + <b>الإزاحة</b> + <b>الرجوع للـ FVG</b> + <b>Order Blocks</b> + <b>Killzones</b>.\n"
        "📌 <b>ملاحظة مهمة:</b> ICT هنا <u>مدرسة مستقلة</u> عن SMC (مش نفس المدرسة)."
    )

    snap = _school_market_header(s)

    checklist = _school_checklist(
        "Checklist سريع قبل أى فكرة دخول (ICT)",
        [
            "هل يوجد <b>Liquidity Sweep</b> واضح (EQH/EQL أو قمم/قيعان قريبة)؟",
            "هل حصل <b>Displacement</b> (شمعة/اندفاع سريع) بعد السحب؟",
            "هل ظهر <b>FVG</b> أو Imbalance يمكن أن يعود السعر لملئه؟",
            "هل يوجد <b>Order Block</b> صالح قريب من منطقة الرجوع؟",
            "هل نحن داخل <b>Killzone</b> مناسبة (لندن/نيويورك)؟",
        ],
    )

    scenarios = (
        "📈 <b>سيناريو صاعد (تعليمى):</b>\n"
        "• Sweep لسيولة القيعان (EQL/قاع قريب) → Displacement صاعد → رجوع لملء FVG/OB → دخول بعد تأكيد شمعة.\n"
        "• الإبطال (Invalidation): كسر قاع الـ OB أو رجوع السعر داخل الفجوة بشكل كامل بدون رفض.\n\n"
        "📉 <b>سيناريو هابط (تعليمى):</b>\n"
        "• Sweep لسيولة القمم (EQH/قمة قريبة) → Displacement هابط → رجوع لمنطقة FVG/OB → دخول بعد تأكيد.\n"
        "• الإبطال: كسر قمة الـ OB أو استرجاع كامل للفجوة بلا رفض."
    )

    risk_line = _school_risk_line(s)

    zones = _zones_block(s)
    if zones:
        zones = "\n\n" + zones

    return "\n\n".join([header, snap, checklist, scenarios, risk_line]) + zones


def _build_school_smc(s: dict) -> str:
    """
    SMC مدرسة Smart Money (بعيد عن ICT): Market Structure + BOS/CHOCH + POIs + Imbalance (بشكل عام).
    """
    header = (
        "🎯 <b>مدرسة SMC — Smart Money</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية شراء/بيع.</i>\n"
        "📌 SMC هنا نركّز على: <b>الهيكلة</b> (HH/HL أو LH/LL) + <b>BOS/CHoCH</b> + مناطق <b>POI</b> (Supply/Demand) + <b>Imbalance</b>.\n"
        "🧠 <b>ملاحظة:</b> SMC هنا <u>مدرسة مستقلة</u> عن ICT (مش نفس المدرسة)."
    )

    snap = _school_market_header(s)

    structure_points = _school_checklist(
        "كيف تقرأ الهيكلة بشكل سريع (SMC)",
        [
            "حدد الاتجاه على فريم أكبر (H4/D1): HH/HL = صعود، LH/LL = هبوط.",
            "ابحث عن <b>BOS</b> (كسر قمة/قاع رئيسى) لتأكيد استمرار الاتجاه.",
            "ابحث عن <b>CHoCH</b> (تغيّر شخصية) كإشارة أولية لانعكاس محتمل.",
            "حدد أقرب <b>POI</b> (Demand/Supply) المتوافقة مع الاتجاه.",
            "راقب <b>Imbalance</b> كفراغ سعرى قد يعود له السعر."
        ],
    )

    scenarios = (
        "📈 <b>سيناريو صاعد (تعليمى):</b>\n"
        "• على فريم كبير: صعود (HH/HL).\n"
        "• هبوط تصحيحى نحو Demand/POI + ظهور رد فعل + كسر BOS صغير على فريم أصغر.\n"
        "• الإبطال: كسر آخر HL أو كسر POI بوضوح.\n\n"
        "📉 <b>سيناريو هابط (تعليمى):</b>\n"
        "• على فريم كبير: هبوط (LH/LL).\n"
        "• صعود تصحيحى نحو Supply/POI + رفض + كسر BOS صغير هابط.\n"
        "• الإبطال: كسر آخر LH أو اختراق POI وتثبيت فوقه."
    )
    ...
    return "\n\n".join([header, snap, structure_points, scenarios, risk_line]) + zones


def _build_school_wyckoff(s: dict) -> str:
    header = (
        "📚 <b>مدرسة Wyckoff — Accumulation / Distribution</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: السوق يتحرك داخل نطاقات (TR)؛ المؤسسات تُراكم ثم تُطلق، أو تُصَرّف ثم تُسقط."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "أهم نقاط Wyckoff (مختصر عملى)",
        [
            "حدد هل السوق داخل <b>Trading Range</b> (قمة/قاع واضحين).",
            "راقب SC/AR/ST فى نهاية الهبوط (تجميع).",
            "راقب UT/UTAD فى نهاية الصعود (تصريف).",
            "انتظر <b>Sign of Strength</b> أو <b>Sign of Weakness</b> قبل الدخول.",
            "ادخل من الأطراف، وليس من منتصف الرينج."
        ],
    )
    scenarios = (
        "📈 <b>تجميع (Bullish):</b> Spring أسفل الرينج + رجوع سريع + اختراق علوي وتثبيت.\n"
        "📉 <b>تصريف (Bearish):</b> UT أعلى الرينج + فشل تثبيت + كسر سفلي مع زيادة ضغط."
    )
    return "\n\n".join([header, snap, checklist, scenarios, _school_risk_line(s)])


def _build_school_harmonic(s: dict) -> str:
    header = (
        "🌀 <b>Harmonic Patterns — XABCD / PRZ</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: نسب فيبو تحدد منطقة انعكاس محتملة PRZ مع تأكيد سلوك السعر."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist هارمونيك",
        [
            "هل يوجد نموذج محتمل (Gartley / Bat / Crab / Butterfly)؟",
            "هل PRZ ضيقة (تلاقى نسب فيبو)؟",
            "هل ظهرت شموع رفض داخل PRZ؟",
            "هل يوجد دعم من هيكلة/سيولة/حجم؟",
            "ضع وقف خسارة خلف PRZ مباشرة."
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_elliott(s: dict) -> str:
    header = (
        "🌊 <b>Elliott Waves — موجات إليوت</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: الاتجاه يتكون من 5 موجات دافعة + 3 تصحيحية؛ نبحث عن العدّ الأقرب للواقع."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "قواعد سريعة (بدون تعقيد)",
        [
            "الموجة 2 لا تكسر بداية الموجة 1.",
            "الموجة 3 ليست الأقصر غالبًا وتكون الأقوى.",
            "الموجة 4 لا تدخل منطقة الموجة 1 (فى العدّ الكلاسيكى).",
            "التصحيح ABC غالبًا يعود لنسب فيبو معروفة.",
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_time(s: dict) -> str:
    header = (
        "⏱ <b>Time Analysis — التحليل الزمنى</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: توقيت الحركة (جلسات/أخبار/دورات) يساوى فى أهميته شكل الحركة."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist زمنى عملى",
        [
            "راقب أوقات السيولة العالية (افتتاح لندن/نيويورك).",
            "تجنب الدخول قبل خبر قوى بدقائق.",
            "راقب تكرار القمم/القيعان فى نفس الأوقات يوميًا.",
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_price_action(s: dict) -> str:
    header = (
        "📈 <b>Price Action — سلوك السعر</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: قراءة الشموع + المستويات + الترند بدون مؤشرات معقدة."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist برايس أكشن",
        [
            "حدد مستويات دعم/مقاومة واضحة.",
            "راقب الشموع الانعكاسية (Pin bar / Engulfing) عند المستويات.",
            "ادخل بعد تأكيد، واعتبر التذبذب حول المستويات إشارة خطر.",
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_sd(s: dict) -> str:
    header = (
        "📦 <b>Supply & Demand — العرض والطلب</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: مناطق Demand/Supply هى أماكن تواجد أوامر قوية؛ نفضل الدخول من المنطقة مع تأكيد."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist S&D",
        [
            "حدد أقرب Demand أسفل السعر وأقرب Supply أعلى السعر.",
            "أفضلية للمناطق التى خرج منها السعر بسرعة (اندفاع).",
            "التأكيد: رفض شموع + كسر صغير للهيكلة على فريم أصغر.",
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_classic(s: dict) -> str:
    header = (
        "🏛 <b>Classical TA — التحليل الكلاسيكى</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: ترندات + قنوات + نماذج + دعم/مقاومة + مؤشرات عامة (اختيارى)."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist كلاسيكى",
        [
            "حدد الترند (Up/Down/Range).",
            "ارسم مستويات دعم/مقاومة رئيسية.",
            "راقب النماذج (Double Top/Bottom, H&S, Triangles).",
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_liquidity(s: dict) -> str:
    header = (
        "💧 <b>Liquidity Map — خريطة السيولة</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: السعر ينجذب لجيوب السيولة (قمم/قيعان متساوية، تجمعات وقف)."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist سيولة",
        [
            "حدد EQH/EQL على الفريمات الصغيرة والمتوسطة.",
            "راقب سحب السيولة ثم انعكاس سريع (stop-hunt).",
            "ادمجها مع هيكلة السوق لتصفية الإشارات."
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_structure(s: dict) -> str:
    header = (
        "🧬 <b>Market Structure — هيكل السوق</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: HH/HL أو LH/LL + مناطق الكسر + التغير فى الشخصية (CHoCH)."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist هيكل",
        [
            "ارسم آخر قمتين وآخر قاعين على الفريم الأكبر.",
            "حدد آخر كسر مهم (BOS).",
            "لو ظهر CHoCH على فريم كبير → قلل مخاطرتك."
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_multi(s: dict) -> str:
    header = (
        "🧭 <b>Multi-Timeframe — تعدد الفريمات</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: اتجاه الفريم الكبير يحدد الانحياز، والفريم الصغير يحدد نقطة الدخول."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Workflow مقترح",
        [
            "D1/H4 لتحديد الاتجاه والمناطق الكبيرة.",
            "H1/M15 لتحديد POI دقيقة.",
            "M5/M1 لتأكيد الدخول بسلوك الشموع."
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_volume(s: dict) -> str:
    header = (
        "📊 <b>Volume & Volatility — الحجم والتقلب</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: ارتفاع الحجم مع التقلب يؤكد قوة الحركة؛ انخفاضهما يشير لتذبذب."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "Checklist حجم/تقلب",
        [
            "لو التقلب مرتفع: قلل الرافعة/الحجم.",
            "لو التقلب منخفض: توقع رينج وتجنب مطاردة الاختراقات.",
            "ادمج مع مستويات واضحة لتجنب إشارات كاذبة."
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def _build_school_risk(s: dict) -> str:
    header = (
        "🧮 <b>Risk & Position — إدارة المخاطر وحجم الصفقة</b>\n"
        "⚠️ <i>تعليمى فقط — ليست توصية.</i>\n"
        "الفكرة: حتى أفضل تحليل بدون إدارة مخاطر = خسارة محتملة."
    )
    snap = _school_market_header(s)
    checklist = _school_checklist(
        "قواعد ذهبية (مختصر)",
        [
            "حدد وقف خسارة قبل الدخول.",
            "لا تخاطر بأكثر من 1%–2% من رأس المال فى صفقة واحدة.",
            "قسّم الدخول/الخروج على مراحل بدل قرار واحد.",
            "لو السوق عنيف (تقلب عالى) → خفّض الحجم."
        ],
    )
    return "\n\n".join([header, snap, checklist, _school_risk_line(s)])


def format_school_report_advanced(code: str, symbol: str = "BTCUSDT") -> str:
    """
    مولّد تقارير مدارس متقدّم (V2).
    - يفصل ICT عن SMC بشكل واضح.
    - يغطّى كل المدارس بلوحة متقدمة + checklists + سيناريوهات.
    """
    c = (code or "").strip().lower()
    sym = _normalize_symbol_for_school(symbol)

    # ALL SCHOOLS: رسالة توضيحية فقط (الواجهة عندك هى inline keyboard)
    if c in ("all", "schools", "all_schools"):
        return (
            "🧠 <b>ALL SCHOOLS</b>\n"
            "اختر مدرسة من اللوحة لعرض تحليلها.\n"
            "• ICT = سيولة/إزاحة/FVG/OB/Killzones (مدرسة مستقلة)\n"
            "• SMC = هيكلة/BOS/CHoCH/POI/Imbalance (مدرسة مستقلة)\n"
            "وباقى المدارس تقدم أسلوب قراءة مختلف للسوق."
        )

    s = _school_snapshot_core(sym)

    if c == "ict":
        return _build_school_ict(s)
    if c == "smc":
        return _build_school_smc(s)
    if c == "wyckoff":
        return _build_school_wyckoff(s)
    if c == "harmonic":
        return _build_school_harmonic(s)
    if c == "elliott":
        return _build_school_elliott(s)
    if c in ("time", "time_analysis"):
        return _build_school_time(s)
    if c in ("price_action", "pa"):
        return _build_school_price_action(s)
    if c in ("sd", "supply_demand", "supply", "demand"):
        return _build_school_sd(s)
    if c in ("classic", "ta", "classical"):
        return _build_school_classic(s)
    if c in ("liquidity", "liq"):
        return _build_school_liquidity(s)
    if c in ("structure", "ms"):
        return _build_school_structure(s)
    if c in ("multi", "mtf"):
        return _build_school_multi(s)
    if c in ("volume", "vol", "volatility"):
        return _build_school_volume(s)
    if c in ("risk", "risk_position", "rm"):
        return _build_school_risk(s)

    # fallback
    return (
        f"📚 <b>تحليل مدرسة:</b> <code>{c.upper()}</code>\n"
        "لا يوجد قالب مفصل لهذه المدرسة بعد، لكن يمكنك اختيار مدرسة من القائمة الأساسية."
    )


# ✅ تفعيل المحرك المتقدم بدون حذف المحرك القديم:
# - لن نحذف format_school_report القديم.
# - سنحوّله تلقائيًا ليستدعى النسخة المتقدمة؛ وبكده كل مكان فى المشروع سيستفيد.
try:
    _OLD_format_school_report = format_school_report  # حفظ المرجع القديم للاحتياط
except Exception:
    _OLD_format_school_report = None

def format_school_report(code: str, symbol: str = "BTCUSDT") -> str:  # type: ignore
    """
    Override آمن:
    - يستخدم المحرك المتقدم V2.
    - لو حصل أى خطأ (نادر) يرجع للمحرك القديم.
    """
    try:
        return format_school_report_advanced(code, symbol=symbol)
    except Exception:
        try:
            if _OLD_format_school_report:
                return _OLD_format_school_report(code, symbol=symbol)
        except Exception:
            pass
        return "⚠️ حدث خطأ أثناء توليد تحليل المدرسة."
