...
# ==============================
#   التحذير الموحد - /alert (MAX + Fusion AI + MAX PRO)
# ==============================


def format_ai_alert() -> str:
    """
    تحذير متطور مبني على:
    ...
    مع إضافة طبقة IN CRYPTO Ai — MAX PRO فى النهاية.
    """
    metrics = compute_market_metrics()
    if not metrics:
        data = fetch_price_data("BTCUSDT")
...
تعذّر جلب قراءات متقدمة للسوق فى هذه اللحظة، لكن حركة البيتكوين الحالية
تشير إلى تقلبات ملحوظة تستدعى الحذر فى اتخاذ قرارات التداول.
...
• {liquidity_pulse}
• محرك المخاطر يشير إلى مستوى: {risk_emoji} {risk_level_text}.
• احتمالية حركة قوية قادمة أعلى من المعتاد (صعود حاد أو كسر دعم مهم).

---

🔵 المدرسة الفنية 

• الصورة الحالية أقرب إلى سوق يميل للهبوط مع محاولات ارتداد ضعيفة.
• شموع الصعود الأخيرة صغيرة مقارنةً بالشموع البيعية، مما يعكس قوة الدببة.
• السعر يتحرك قرب مناطق تعتبر Discount نسبياً، لكن بدون دخول قوى واضح من المشترين حتى الآن.
• سلوك السعر خلال الساعات الأخيرة يميل إلى التذبذب فى نطاق ضيق لصالح البائعين.

---

🟣 الهارمونيك (Harmonic View)

• نموذج حركى أقرب إلى ABCD هابط قيد التكوين.
• منطقة الانعكاس المحتملة (PRZ) تُقدَّر قرب مناطق الدعم الأخيرة أسفل السعر الحالى.
• يحتاج تأكيد واضح من شمعة ارتداد قوية قبل اعتبار النموذج فعالاً.
• أى ارتداد بدون حجم تداول داعم قد يكون مجرد Bounce قصير وليس انعكاسًا حقيقيًا للاتجاه.

---

📉 المؤشرات الفنية

• قراءة RSI التقديرية حوالى: {rsi:.1f} → {rsi_trend}.
• درجة التقلب اليومية {volatility_score:.1f} / 100 تشير إلى سوق غير مستقر بالكامل.
• الزخم العام يميل للضعف مع غياب إشارات انعكاس مكتملة حتى الآن.
• أحجام التداول الحالية لا تعكس دخول قوى من سيولة جديدة، مما يجعل أى ارتداد محتمل معرضًا للضعف.

---

🔗 بيانات الـ On-Chain (تصوّر ذكى مبنى على حركة السوق)

• نماذج السيولة الذكية تشير إلى وجود ضغوط بيعية مؤسسية ملحوظة فى الأيام الأخيرة.
• خروج تدريجى للسيولة من مناطق السعر الأعلى إلى مناطق أقل، بدون إشارات FOMO شرائى قوى.
• سلوك المحافظ الكبيرة (محاكاة AI) يميل إلى تقليل المخاطرة على المدى القصير، وليس الخروج الكامل من السوق.
• الصورة العامة ترجّح أن السوق فى طور إعادة توزيع أكثر من كونه فى موجة صعود قوية مكتملة.

---

💎 استثماريًا (مدى متوسط)

• من منظور متوسط المدى، لا يُفضّل اعتبار الاتجاه استثمارى صاعد إلا بعد اختراق واضح لمناطق مقاومة قوية أعلى السعر الحالى.
• أى إغلاق يومى واضح أعلى من قمم الأيام الماضية يعزز فكرة أن التصحيح الحالى انتهى.
• كسر مناطق دعم رئيسية أسفل السعر الحالى قد يفتح الباب لتصحيحات أعمق قبل استكمال دورة الصعود الكبرى.

---

⚡ مضاربيًا (قصير المدى)

• التداول برافعة عالية فى هذه المرحلة يحمل مخاطرة مرتفعة بسبب التقلب وعدم وضوح الاتجاه النهائى.
• أفضل أسلوب حالياً هو تقليل حجم العقود والتركيز على صفقات قصيرة المدى مع وقف خسارة صارم.
• مناطق الدعم والمقاومة القريبة من السعر الحالى قد تولّد ارتدادات سريعة لكن بدون ضمان استمرار الحركة.
• أى كسر قوى لمناطق دعم قريبة مع زيادة فى الحجم البيعى قد يسرّع من الهبوط على المدى اللحظى.
...
• توصية النظام:
  ...
  → التركيز على حماية رأس المال أولاً.
  → انتظار تأكيدات أوضح من السعر والحجم قبل أى دخول عدوانى.
""".strip()

    # 🔥 بلوك IN CRYPTO Ai — MAX PRO فى آخر التحذير
    ai_final_block = f"""
🧠 <b>IN CRYPTO Ai — MAX PRO</b>
نظام ذكاء اصطناعى متعدد الطبقات مخصص لتحليل السوق فى الوقت الفعلى بدقة عالية.

🔎 <b>نظرة النظام على السوق الآن:</b>
• <b>اتجاه الحركة:</b> {fusion['bias_text']}
• <b>سلوك السيولة:</b> {fusion['liquidity']}
• <b>المرحلة الحالية للسوق:</b> {fusion['wyckoff_phase']}
• <b>تقييم المزاج العام:</b> {fusion['risk_comment']}

📊 <b>تقديرات الحركة (24–72 ساعة):</b>
- صعود محتمل: <b>~{fusion['p_up']}%</b>  
- تماسك جانبى: <b>~{fusion['p_side']}%</b>  
- هبوط محتمل: <b>~{fusion['p_down']}%</b>

🔬 <b>قراءة سلوك السوق (SMC Pro):</b>
• السوق يظهر إشارات {fusion['smc_view']}  
• التفاعل الحالى مع مناطق السيولة يشير إلى انتقال منظم بين مناطق العرض والطلب.

📈 <b>القراءة الديناميكية:</b>
• النظام يراقب تغيّر الزخم الفعلى لحظة بلحظة.  
• أى اختراق لمستوى سيولة رئيسى يمكن أن يعيد تشكيل خريطة الاحتمالات بسرعة.  
• التقلب الحالى: <b>يتفاعل مباشرة مع نموذج المخاطر الموحّد داخل النظام.</b>

🏁 <b>خلاصة IN CRYPTO Ai — MAX PRO:</b>
السوق حاليًا فى نقطة حساسة تُبنى فيها القرارات على استجابة السعر لمناطق السيولة القريبة.  
الاتجاه القادم سيتحدد عبر ثلاثة محاور رئيسية:
1) مستوى الاختراق القادم،  
2) حجم السيولة الفعال،  
3) قوة الزخم المصاحب للحركة.

<b>IN CRYPTO Ai 🤖 — MAX PRO Engine</b>
""".strip()
