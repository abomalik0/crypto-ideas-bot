...
if lower_text == "/start":
    ...
    user_block = (
        ...
        "منظومة ذكاء اصطناعى تتابع حركة البيتكوين لحظيًا وتقدّم لك رؤى جاهزة بدون تعقيد.\n\n"
        
        "📌 <b>أوامر المستخدم الأساسية:</b>\n"
        "• <code>/btc</code> — تحليل لحظى لـ BTC\n"
        "• <code>/coin btc</code> — تحليل أى عملة (مثال: <code>/coin eth</code> / <code>sol</code>)\n"
        "• <code>/market</code> — نظرة عامة على حالة السوق\n"
        "• <code>/risk_test</code> — اختبار بسيط لإدارة المخاطر\n\n"

        "💡 <b>معلومات مهمة:</b>\n"
        "• البيانات معتمدة على أسعار Binance مع معالجة وفلترة دقيقة.\n"
        "• التحليل تعليمى ومساعد لاتخاذ القرار — وليس توصية مباشرة.\n"
    )
    ...
    if is_admin:
        admin_block = (
            "\n\n👑 <b>أهلاً بك فى IN CRYPTO Ai — لوحة الإدارة</b>\n\n"

            "⚙️ <b>أوامر الإدارة:</b>\n"
            "🔸 <code>/alert</code> — إرسال Ultra PRO Alert فورًا للجميع\n"
            "🔸 <code>/test_smart</code> — عرض Smart Alert Snapshot\n"
            "🔸 <code>/status</code> — حالة النظام الكاملة\n"
            "🔸 <code>/weekly_now</code> — إرسال التقرير الأسبوعى الآن\n\n"

            "👑 <b>إدارة الصلاحيات (Owner فقط):</b>\n"
            "🔹 <code>/add_admin &lt;chat_id&gt;</code> — إضافة أدمن\n"
            "🔹 <code>/remove_admin &lt;chat_id&gt;</code> — إزالة أدمن\n\n"

            "🖥️ <b>لوحة التحكم:</b>\n"
            f"• <a href=\"{config.APP_BASE_URL}/admin/dashboard?pass=ahmed123\">اضغط هنا لفتح Dashboard</a>\n"
        )

    # إرسال الرسالة المناسبة حسب نوع المستخدم
    welcome_message = user_block + admin_block
