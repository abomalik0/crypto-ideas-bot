...
def format_market_report() -> str:
    ...
    fusion_line = (
        f"- قراءة IN CRYPTO Ai: {fusion['bias_text']} | {fusion['smc_view']} | {fusion['wyckoff_phase']}"
    )
...
# ==============================
#   التحذير الموحد - /alert (قصير + احترافى + IN CRYPTO Ai)
# ==============================
...
def format_ai_alert() -> str:
    ...
    مع الحفاظ على طول مناسب لـ Telegram.
    """
    metrics = compute_market_metrics()
    if not metrics:
        data = fetch_price_data("BTCUSDT")
...
<b>IN CRYPTO Ai — Weekly Intelligence Report</b>
...
📌 <b>تعليق IN CRYPTO Ai:</b>
"السوق فى مرحلة انتقالية… شمعة اختراق أو كسر قوية قد تعيد رسم الخريطة بالكامل."

---

🔵 <b>القسم 2 — قراءة فنية مبسطة (BTC)</b>

- السوق يتحرك داخل نطاق يتسم بـ:
  • تذبذب محسوب  
  • سيولة متوسطة إلى جيدة  
  • حساسية عالية للأخبار

- مؤشر RSI يميل لمنطقة الحياد مع ميل بسيط للإيجابية.
- الزخم يتحسن تدريجيًا لكن لم يصل بعد لمرحلة صعود حاد مؤكد.
...
- حركة ETH ما زالت مرتبطة بقرار البيتكوين.
- لا تظهر حاليًا إشارات قيادة مستقلة للسوق من الإيثيريوم.
- أى اختراق قوى فى BTC غالبًا ينعكس سريعًا على ETH.

---

🟩 <b>القسم 4 — السيولة والمزاج العام</b>

- نبض السيولة: {liquidity_pulse}
- حالة المزاج العام (Sentiment تقريبى):
  • المشترون (Bullish): ~{bulls}%  
  • البائعون (Bearish): ~{bears}%  
  • الحياد / الانتظار: ~{neutral}%

📌 <b>ملخص المزاج:</b>
"السوق ليس فى حالة ذعر ولا حالة فقاعة؛ مزاج متزن مع ميل بسيط للإيجابية."

---

🧠 <b>القسم 5 — تقديرات IN CRYPTO Ai للأسبوع القادم</b>

- صعود محتمل: ~<b>{p_up}%</b>
- تماسك / تذبذب جانبى: ~<b>{p_side}%</b>
- هبوط محتمل: ~<b>{p_down}%</b>

📌 <b>قراءة مركّزة:</b>
- اختراق قمم الأسبوع الحالى بإغلاق واضح يعزز سيناريو صاعد أقوى.
- كسر الدعوم الرئيسية قد يفتح باب لتصحيح أعمق قبل استكمال دورة الصعود.

---

⚠️ <b>القسم 6 — المخاطر وإدارة القرارات</b>

- درجة التقلب الحالية: <b>{vol:.1f} / 100</b>
- مستوى المخاطر العام: {risk['emoji']} <b>{risk_level_text}</b>
- التوصية العامة:
  • تجنب استخدام رافعة عالية بدون خطة صارمة  
  • التركيز على نقاط الدخول الواضحة فقط  
  • اعتبار وقف الخسارة جزء من الصفقة وليس خيار إضافى

---

🟢 <b>القسم 7 — الخلاصة النهائية من IN CRYPTO Ai</b>

"البيتكوين يقف أمام مفترق طرق سعري مهم.  
البيانات لا تُظهر ذعرًا ولا فقاعة؛ بل سوقًا يُبنى بهدوء.  

الأسابيع القادمة ستكون مرتبطة بقوة باختراق أو كسر المناطق الحالية.  
من ينجح هو من يوازن بين الصبر وإدارة المخاطر، وليس من يطارد كل حركة."
